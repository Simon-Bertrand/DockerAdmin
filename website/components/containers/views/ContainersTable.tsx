...
import { TabList, Tab, Card, Flex, Grid } from "@tremor/react";
...
const groupRawData = (ans: IContainers[]|undefined): { [key: string]: IContainers[] } => {
...
const getNumberOfContainers = (data: { [key: string]: IContainers[] }) => {
  return Object.keys(data).map(x => data[x].length).reduce((prev, curr) => prev + curr, 0)
}

export default function ContainersTableComponent() {
  const { payload, refetch } = containersApi.getContainers()
  const data = groupRawData(payload)
  const [tab, setTab] = useState<string>("");
  useEffect(() => {
    setTab(Object.keys(data).length >0 ? Object.keys(data)[0]:"")
  }, [data])


  if (!payload) { return (<SpinnerComponent />) }
  if (payload.length == 0) { return (<h3>No containers on the system</h3>) }
  ...
  return (
    <Card className="card-color-1">
      <>
        <h1>System</h1>
        <h5>{getNumberOfContainers(data)} detected containers</h5>
        <TabList
          value={tab}
